...
public class PlantDataUtils {
            ...
            throws PlantNoAgeableInterfaceException, PlantSettingNotFoundException
    {
        ...
        SettingsPlantGrowthDTO settings = PlantDataManager.getInstance()
                                                          .getSettingsDataBase()
                                                          .getPlantSettings(ipc.getDatabasePlantType(plant.getLocation()
                                                                                                          .getBlock()));
        ...
        while (currentTimeStamp >= previousUpdateTime && tempSimulatedAge < realMaximumAge) {
            final long increaseBy = !settings.isUseTimeForPlantMature() &&
                                    ...
                                    (TimeUnit.MILLISECONDS.convert(
                                            settings.getTimeForNextPlantGrowthInSteps().get(tempSimulatedAge),
                                            TimeUnit.SECONDS
                                    )) :
                                    (TimeUnit.MILLISECONDS.convert(
                                            settings.getTimeForPlantMature() == null ?
                                            1 :
            ...
            Bukkit.getLogger()
                  .log(
                          Level.FINER,
                          MessageFormat.format(
                                  "{0} update step increased by {1}ms (AGE: {3}/{2} [{4}])",
                                  plant.getPlantType(),
                                  increaseBy,
                                  realMaximumAge,
                                  tempSimulatedAge,
